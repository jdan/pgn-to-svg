...
export default function pgnToSvg({
  ...
  if (pgn) {
    chess.load_pgn(pgn);
    const details = chess.history({ verbose: true }).reverse()[0];
    lastMoveFrom = details.from;
    lastMoveTo = details.to;
  }

  chess.board().map((row, y) => {
    row.map((piece, x) => {
      elements.push(
        `<rect xmlns="http://www.w3.org/2000/svg" width="45" height="45" x="${
          x * SQUARE_SIZE
        }" y="${y * SQUARE_SIZE}" fill="${
          x % 2 == y % 2 ? lightSquareColor : darkSquareColor
        }"/>`
      );
      ...
      if (
        ...
        elements.push(
          `<rect xmlns="http://www.w3.org/2000/svg" width="45" height="45" x="${
            x * SQUARE_SIZE
          }" y="${y * SQUARE_SIZE}" fill="${lastMoveHighlight}"/>`
        );
      }

      if (piece) {
        const key = (piece.color + piece.type) as keyof typeof pieces;
        elements.push(pieces[key](x * SQUARE_SIZE, y * SQUARE_SIZE));
      }
    });
  });

  return `<svg xmlns="http://www.w3.org/2000/svg" version="1.1" xmlns:xlink="http://www.w3.org/1999/xlink" width="${
    8 * SQUARE_SIZE
  }" height="${8 * SQUARE_SIZE}">${elements.join("")}</svg>`;
}
